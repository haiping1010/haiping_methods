...
train_Y=[]
for name in arr_frr:
    mol = Chem.MolFromPDBFile('all_data/'+name.strip().replace('_poc.pdb','.pdb'), removeHs=not False)
...
class TestbedDataset(InMemoryDataset):
    ...
    def process(self, xd, pocket_graph, y, smile_graph):
        ...
        for i in range(data_len):
            ...
            c_size, features, edge_index,edge_attrs =  pdb_graph(filename, 'all_data/'+filename.replace("_poc.pdb",".pdb"))
            #print (smile_graph[str(filename[0:4])])
