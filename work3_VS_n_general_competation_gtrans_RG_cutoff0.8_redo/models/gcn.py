...
class GCNNet(torch.nn.Module):
    ...
    def __init__(self, n_output=1, n_filters=64, embed_dim=1280,num_features_xd=78, num_features_xt=30, output_dim=1280, dropout=0.2):
        ...
        self.conv1 = TransformerConv(num_features_xd, num_features_xd,heads=4, concat=True, dropout=0.1)
        self.conv2 = TransformerConv(num_features_xd*4, num_features_xd*4, heads=4, concat=True, dropout=0.1)
        #self.conv3 = GCNConv(num_features_xd*2*4, num_features_xd * 4)
        #self.conv4 = GCNConv(num_features_xd*4, num_features_xd * 4)
        self.fc_g1 = torch.nn.Linear(num_features_xd*4*4, 1024)
        self.fc_g2 = torch.nn.Linear(1024, output_dim)
        self.relu = nn.ReLU()
        self.dropout = nn.Dropout(dropout)

        # protein sequence branch (1d conv)
        self.conv1_xt = TransformerConv(num_features_xt, num_features_xt,heads=4, concat=True, dropout=0.1)
        self.conv2_xt = TransformerConv(num_features_xt*4, num_features_xt*4,heads=4, concat=True, dropout=0.1)
        #self.conv3_xt = GCNConv(num_features_xt*2*4, num_features_xt * 4)
        #self.conv4_xt = GCNConv(num_features_xt*4, num_features_xt * 4)
        self.fc_g1_xt = torch.nn.Linear(num_features_xt*4*4, 1024)
    ...
    def forward(self, data, TRAIN_BATCH_SIZE, device):
        ...
        for xx in train_loader2:
                ...
                xx.to(device)
                # (xx[0].x)
                xt, edge_index_t,batch_t=xx.x, xx.edge_index,xx.batch
