...
#compound_iso_smiles = []
import glob
arr_name_f=glob.glob("all_data/*.smi")

compound_dic = {}

arr_name=[]

for name in  arr_name_f:
   fr=open(name,'r')
   arr=fr.readlines()
   linearr=arr[0].strip()
   #compound_iso_smiles.append(linearr[0])
   #print (linearr[0])
   compound_dic[name.replace('all_data/','')] =smile_to_graph(linearr)
   mol = Chem.MolFromSmiles(linearr)
   ...
   if  mol is not None:
        ...
        if num_bonds>0:
            ...
            arr_name.append(name.replace('all_data/',''))
...
train_data = TestbedDataset(root='data1', dataset='L_P_train_'+pdbname.replace('.pdb',''), xd=arr_name, pocket_graph=pocket_graph, y=train_Y, compound_dic=compound_dic)
